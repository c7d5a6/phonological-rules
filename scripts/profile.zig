...
pub fn main(init: std.process.Init) !void {
    ...
    try w.print("corpus\tphase\titers\tns/op\n", .{});
...
fn timeSoundParse(io: Io, w: *Io.Writer, case: Case) !void {
    ...
    try printRow(w, case.name, "sound parse", parse_iters, nsPerOp(elapsedNs(io, start), parse_iters));
}
...
fn timeRuleParse(a: Allocator, io: Io, w: *Io.Writer, case: Case) !void {
    ...
    try printRow(w, case.name, "rule parse", parse_iters, nsPerOp(elapsedNs(io, start), parse_iters));
}
...
fn timeMatch(a: Allocator, io: Io, w: *Io.Writer, case: Case) !void {
    ...
    try printRow(w, case.name, "match", match_iters, nsPerOp(elapsedNs(io, start), match_iters));
}
...
fn timeApply(a: Allocator, io: Io, w: *Io.Writer, case: Case) !void {
    ...
    try printRow(w, case.name, "apply", apply_iters, nsPerOp(elapsedNs(io, start), apply_iters));
}
